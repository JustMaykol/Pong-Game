...
void DuoGame::draw(RenderWindow &window) {
...
ostream &operator<<(ostream &output, const DuoGame &game) {
    auto t = time(nullptr);
    auto lt = *localtime(&t);

    output << "DuoGame [" << put_time(&lt, "%m:%d:%Y %H:%M:%S") << "]" << endl << endl;
